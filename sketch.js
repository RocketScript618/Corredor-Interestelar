...
var highscore,score;
var laser,asteroid;
var shield_imp,hull_imp,shutdown,explosion;
var music1;
highscore = 0;
...
function preload() {
  ...
  // NO TOMO CREDITO POR LA MUSICA 
  shield_imp = new Audio("shield_impact.mp3");
  hull_imp = new Audio("hull_impact.mp3");
  shutdown = new Audio("shutdown.mp3");
...
function draw() {
  ...
  if(gameState=="stand-by"){
    ...
    fill("yellow");
    text("Se sugiere bajar el volumen",90,248);
    ship.visible=false;
...
function ast_spawn(){
  if(gameState=="play"){
    if(frameCount % 15 == 0){
      ...
      asteroid.depth = statsHolder.depth;
      statsHolder.depth = statsHolder.depth+1;
      hull_show.depth = statsHolder.depth;
      hull_dis.depth = statsHolder.depth;
      shield_show.depth = statsHolder.depth;
      shield_dis.depth = statsHolder.depth;
